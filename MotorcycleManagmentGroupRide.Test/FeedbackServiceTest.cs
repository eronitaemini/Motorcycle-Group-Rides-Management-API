...
namespace MotorcycleGroupRidesManagement.Tests
    ...
    public class FeedbackServiceTest
    {
        ...
        [Fact]
        public async Task DeleteFeedbackAsync_InvalidId_DoesNotDeleteFeedback()
        {
            var feedbackId = Guid.NewGuid();
            _mockRepo.Setup(repo => repo.GetByIdAsync(feedbackId)).ReturnsAsync((Feedback)null);

            await _service.DeleteFeedbackAsync(feedbackId);

            _mockRepo.Verify(repo => repo.DeleteAsync(It.IsAny<Guid>()), Times.Never);
            _mockRepo.Verify(repo => repo.SaveChangesAsync(), Times.Never);
        }


        [Fact]
        public async Task DeleteFeedbackAsync_ValidId_DeletesFeedback()
        {
            var feedbackId = Guid.NewGuid();
            var feedback = new Feedback { FeedbackId = feedbackId, Comments = "To be deleted" };
            _mockRepo.Setup(repo => repo.GetByIdAsync(feedbackId)).ReturnsAsync(feedback);
        ...
        [Fact]
        public async Task UpdateFeedbackAsync_InvalidId_ThrowsKeyNotFoundException()
        {
            var feedbackId = Guid.NewGuid();
            var updateFeedbackDto = new UpdateFeedbackDto { Comments = "Updated comment", Rating = 4 };
            _mockRepo.Setup(repo => repo.GetByIdAsync(feedbackId)).ReturnsAsync((Feedback)null);

            await Assert.ThrowsAsync<KeyNotFoundException>(() => _service.UpdateFeedbackAsync(feedbackId, updateFeedbackDto));

            _mockRepo.Verify(repo => repo.UpdateAsync(It.IsAny<Feedback>()), Times.Never);
            _mockRepo.Verify(repo => repo.SaveChangesAsync(), Times.Never);
        }
        ...
        [Fact]
        public async Task GetFeedbacksAsync_ReturnsFilteredSortedAndPagedFeedbacks()
        {
            var feedbackList = new List<Feedback>
        {
            new Feedback { FeedbackId = Guid.NewGuid(), Comments = "Good", Rating = 4 },
            new Feedback { FeedbackId = Guid.NewGuid(), Comments = "Great", Rating = 5 }
        };
            _mockRepo.Setup(repo => repo.GetFeedbacksAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<bool>(), It.IsAny<int>(), It.IsAny<int>(), It.IsAny<int?>(), It.IsAny<int?>()))
                     .ReturnsAsync(feedbackList);

            var result = await _service.GetFeedbacksAsync("Good", "Rating", true, 1, 10, 3, 5);
